...
function getTxtString(layer, deepIndex) {
    if(layer.kind == LayerKind.TEXT){
        ...
        txtid = txtid.replace(/\s+/g,"")//替换所有空格
...
function savePicture(layer){
    ...
    var tmpcoords = getXYWH (layer)
    //var deltax = 0-tmpcoords[0];
    //var deltay = 0-tmpcoords[1];
    ...
    picName = picName.replace(/\s+/g,"")//替换所有空格
    ...
    // fileName = fileName.replace(/\s+/g,"")//替换所有空格
...
function saveScalePiture(layer){
    ...
    if(layer.name=="ui_kfhd_di_guangqia@42_42_42_42"){
        $.writeln ("ui_kfhd_di_guangqia@42_42_42_42-->" + picName);
        $.writeln ("ui_kfhd_di_guangqia@42_42_42_42-->" + picName);
    ...
    // scaleDocName = scaleDocName.replace(/\s+/g,"")//替换所有空格
    //定义一个变量[file]，作为图层输出的路径。
...
function getImageName(layer) {
    ...
    sourceName = sourceName.replace(/\s+/g,"")//替换所有空格
    return sourceName
}
